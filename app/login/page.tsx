...
export default function Login({ searchParams }: { searchParams: { message: string } }) {
  ...
  const signUp = async (formData: FormData) => {
    ...
    // Optionally redirect to a page asking the user to check their email for confirmation
    // For now, redirect back to login with a success message (if email confirmation is disabled)
    return redirect('/login?message=Signup successful! Please log in.') // Adjust if email confirmation is needed
    ...
    <div className="flex-1 flex flex-col w-full px-8 sm:max-w-md justify-center gap-2 mx-auto">
      ...
      >
          ...
          <div className="flex flex-col sm:flex-row sm:justify-center gap-4">
              <Button formAction={signIn} variant="default" className="w-full sm:w-auto">
                  Sign In
              </Button>
              <Button formAction={signUp} variant="outline" className="w-full sm:w-auto">
                  Sign Up
              </Button>
          </div>
